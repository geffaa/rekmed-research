...
class PasienController extends Controller
{
    ...
    public function actionDiaryMobile($id){
        ...
        if($model->count()==0)
            return "Maaf, Pasien belum pernah menulis pada Diary Pasien, Sarankan Pasien Anda mendownload <a href='https://play.google.com/store/apps/details?id=id.gamatutor.pasien.diarypasien' target='_blank'>aplikasi rekmed untuk pasien</a>, untuk komunikasi dengan dokter";
        else{
            $data = $model->all();
    ...
    public function actionCreate($asal=null)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $model->mr = $model->klinik_id.substr($model->mr, 0,6);
            ...
            if(isset(Yii::$app->request->post()['Antrian'])){
                ...
                $kunjungan->created = date('Y-m-d H:i:s');
                $kunjungan->status = 'antri';
                $kunjungan->dokter_periksa = Yii::$app->request->post()['Kunjungan']['dokter_periksa'];
    ...
    public function errorsToList($error)
    {
        $str = 'Error List: <ul>';
